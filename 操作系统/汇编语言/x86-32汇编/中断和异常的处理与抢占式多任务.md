...
SECTION  mbr  vstart=0x00007c00         
       ...
       ;基地址为 0，界限 0xFFFFF，DPL=00，S=1，4KB粒度，D=1 表示 32 位的偏移地址或操作数，可读可写数据段描述符，向上扩展
...
general_interrupt_handler:
       ...
       ;当 8259A 响应某一级中断而为其服务时，中断服务寄存器 ISR 的相应位置 1，在中断服务结束时，ISR 中相应位应清 0，以便再次接收同级别的中断
       push eax
       ;中断结束命令 EOI
       mov al,0x20
       ;向从片发送 
...
rtm_0x70_interrupt_handle:
       ...
       mov al,0x0c
       out 0x70,al
       in al,0x71                         ;读一下 RTC 的寄存器 C，否则只发生一次中断
                                          ;此处不考虑闹钟和周期性中断的情况
