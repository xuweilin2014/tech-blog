...
```c{.line-numbers}
monica@monica-virtual-machine:~/linkers_loaders$ gcc -S a.c -m32 -fno-stack-protector
...
```c{.line-numbers}
monica@monica-virtual-machine:~/linkers_loaders$ gcc -S b.c -m32 -fno-stack-protector
...
00001020 <main>:
    ...
    1046:	8d 90 10 00 00 00    	lea    0x10(%eax),%edx
    104c:	52                   	push   %edx
...
接下来是 **`call  __x86.get_pc_thunk.ax`** 指令，将下一条指令的地址保存到 **`%eax`** 寄存器中，本条指令的重定位类型是 **`R_386_PC32`**，这种重定位方式指令修改正方式每个被修正的位置的长度都为 32 位，即 4 个字节。
...
**`shared@GOT`** 表示 GOT 中保存 **`shared`** 这个变量相应的项与 GOT 基地址（**`$_GLOBAL_OFFSET_TABLE_`**）之间的偏移量/距离。
