...
create_copy_cur_pdir:                       
        ...
        ;ESI->当前页目录的线性地址
        mov esi,0xfffff000
        ;EDI->新页目录的线性地址（目前 CR3 还是指向内核页目录）
        mov edi,0xffffe000
        ;ECX=要复制的目录项数
        mov ecx,1024
        cld
...
SECTION core_data vstart=0
        ...
        ;page_bit_map 
        page_bit_map    db  0xff,0xff,0xff,0xff,0x03,0x00,0x00,0x00
...
.b3:
        ...
        ;eax=线性基地址，ebx=段界限，ecx=属性，创建的段描述符安装在 edx:eax 中
        call sys_routine_seg_sel:make_seg_descriptor
        ;TCB 的基地址
        mov ebx,esi
        ;输入为 edx:eax=创建的段描述符，ebx=TCB 基地址，输出为 cx=描述符的选择子
...
.b3:
        ...
        ;将 GDT 中的基地址（32 位）和界限（16 位）保存到数据段 pgdt 标号处，并将基地址移动到 ebx 寄存器
        sgdt [pgdt]
        mov ebx,[pgdt+2]
...
.b4:
        ...
        mov ecx,ebx
        ;将此 TCB 添加到 TCB 链中，但是该 TCB 链的作用在后面进行抢占式任务切换时才能显现出来，输入 ecx 为 TCB 线性基地址
